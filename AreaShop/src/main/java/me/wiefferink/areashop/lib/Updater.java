package me.wiefferink.areashop.lib;
...
public class Updater {
	...
	private void moveNewZipFiles(String zipPath) {
		...
		for(final File ddFile : list) {
			if(ddFile.isDirectory() && this.pluginExists(ddFile.getName())) {
				// Current dir
				final File oFile = new File(this.plugin.getDataFolder().getParent(), ddFile.getName());
				// List of existing files in the new dir
				final File[] dList = listFilesOrError(ddFile);
				// List of existing files in the current dir
				final File[] oList = listFilesOrError(oFile);
				for(File ccFile : dList) {
					// Loop through all the files in the new dir
					boolean found = false;
					for(final File xxFile : oList) {
						// Loop through all the contents in the current dir to see if it exists
						if(xxFile.getName().equals(ccFile.getName())) {
							found = true;
							break;
						}
					}
					if(!found) {
						...
						File output = new File(oFile, ccFile.getName());
						this.fileIoOrError(output, ccFile.renameTo(output), true);
					} else {
						// This file already exists, so we don't need it anymore.
						this.fileIoOrError(ccFile, ccFile.delete(), false);
					}
				}
			}
			this.fileIoOrError(ddFile, ddFile.delete(), false);
		}
